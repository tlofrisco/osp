...
class DynamicActivityFactory {
  ...
  private static async createEntity(step: WorkflowStep, input: any) {
    ...
    if (step.target_entity === 'Reservation') {
      ...
      if (Object.keys(input).length === 0 || !input.customer_name) {
        console.log('⚠️ No input provided - generating test data for workflow execution');
    ...
    console.log(`💾 Creating ${entityName} with data:`, entityData);
    
    // 🔥 USE RPC PATTERN (same as API endpoint)
  ...
  private static async updateEntity(step: WorkflowStep, input: any) {
    ...
    console.log(`🔄 Updating ${entityName} ${input.entityId} with:`, updateData);

    // 🔥 USE RPC PATTERN FOR UPDATES
    // Extract service schema from workflow context
    const serviceSchema = input.serviceSchema || input._serviceSchema || 'a_restaurant_reservation_syste_1749301938153';
  ...
  private static async checkAvailability(step: WorkflowStep, input: any) {
    ...
    console.log(`🔍 Checking availability for ${entityName}`);
    ...
    console.log(`🔍 Executing availability query: ${querySql}`);
    console.log(`🔍 With values:`, filterValues);
    ...
    console.log(`✅ Availability check result: ${isAvailable ? 'Available' : `${conflicts.length} conflicts found`}`);
...
function createDynamicWorkflow(workflowDef: WorkflowDefinition) {
  return async function dynamicWorkflow(input: any) {
    ...
    console.log(`🏗️ Using service schema: ${serviceSchema}`);
  ...
  try {
    ...
    workflows.forEach((workflow: WorkflowDefinition) => {
      ...
      workflow.steps.forEach(step => {
        ...
        console.log(`  📝 Registered activity: ${activityKey}`);
      });
    });
    
    // 🔥 USE ACTUAL TEMPORAL CLOUD CONNECTION
    ...
    console.log(`🌐 Connected to Temporal Cloud namespace: quickstart-osp.v5egj`);
    console.log(`📦 Registered ${Object.keys(workflowsMap).length} workflows`);
    console.log(`⚙️ Registered ${Object.keys(activities).length} activities`);
